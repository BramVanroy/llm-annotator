...
def test_step_client_is_merged_once_per_step(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Validation, --describe-steps and the run all ask for a step's client;
    # the merge and its validation happen once.
    merges = 0
    original = ClientConfig.model_validate

    def counting_validate(data: Any, **kwargs: Any) -> ClientConfig:
        nonlocal merges
        merges += 1
        return original(data, **kwargs)

    monkeypatch.setattr(
        ClientConfig, "model_validate", staticmethod(counting_validate)
    )
    config = PipelineConfig.model_validate(
        minimal_config(
            steps=[{"name": "a", "prompt": "x", "client": {"batch_size": 4}}]
        )
    )
    merged = config.step_client(config.steps[0])
    config.describe_steps()

    assert merges == 1
    assert config.step_client(config.steps[0]) is merged
