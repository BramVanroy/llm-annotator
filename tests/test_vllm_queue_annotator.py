...
def test_requires_at_least_one_client() -> None:
    ...
    with pytest.raises(ValueError, match="at least one vLLM server client"):
...
def test_rejects_non_vllm_clients() -> None:
    ...
    class NotVLLM(FakeVLLMOnlineClient):
...
def test_set_queue_size_resolves_like_the_constructor() -> None:
    ...
    annotator.set_queue_size(1)
    assert annotator.queue_size == 3
    annotator.set_queue_size(None)
    assert annotator.queue_size == 12

    with pytest.raises(ValueError, match="positive integer"):
        annotator.set_queue_size(0)
...
def test_annotates_every_sample_exactly_once(
    ...
    clients = [
        FakeVLLMOnlineClient(base_url=f"http://w{i}") for i in range(n_clients)
    ]
...
def test_results_are_sorted_by_idx(tmp_path: Path) -> None:
    ...
    annotator = VLLMQueueAnnotator(
        clients=[
            FakeVLLMOnlineClient(base_url=f"http://w{i}") for i in range(4)
        ],
        batch_size=3,
    )
...
def test_queue_size_bounds_in_flight_batches(tmp_path: Path) -> None:
    ...
    class CountingClient(FakeVLLMOnlineClient):
...
def test_output_columns_match_the_base_annotator(tmp_path: Path) -> None:
    ...
    base = Annotator(client=FakeVLLMOnlineClient(), batch_size=2)
    ...
    queued = VLLMQueueAnnotator(
        clients=[FakeVLLMOnlineClient(), FakeVLLMOnlineClient()], batch_size=2
    )
...
def test_keep_columns_are_written_through(tmp_path: Path) -> None:
    ...
    annotator = VLLMQueueAnnotator(
        clients=[FakeVLLMOnlineClient()], batch_size=2
    )
    result = annotator.run_annotation(
        output_dir=tmp_path / "out",
...
def test_short_client_response_is_rejected(tmp_path: Path) -> None:
    ...
    annotator = VLLMQueueAnnotator(
        clients=[FakeVLLMOnlineClient(truncate_responses=1)], batch_size=4
    )
...
def test_crash_and_resume_has_no_gaps_or_duplicates(tmp_path: Path) -> None:
    ...
    crashing = [
        FakeVLLMOnlineClient(base_url=f"http://w{i}", fail_after_n_batches=2)
        for i in range(2)
    ]
    ...
    healthy_annotator = VLLMQueueAnnotator(
        clients=[
            FakeVLLMOnlineClient(base_url=f"http://w{i}") for i in range(2)
        ],
        batch_size=5,
    )
...
def test_fully_processed_run_short_circuits(tmp_path: Path) -> None:
    ...
    second_client = FakeVLLMOnlineClient()
...
def test_annotate_dataset_entry_point(tmp_path: Path) -> None:
    ...
    clients = [FakeVLLMOnlineClient(base_url=f"http://w{i}") for i in range(2)]
    annotator = VLLMQueueAnnotator(clients=clients, batch_size=2)
...
def test_destroy_continues_after_a_failing_client() -> None:
    ...
    class BrokenClient(FakeVLLMOnlineClient):
        def destroy(self) -> None:
            super().destroy()
            raise RuntimeError("cannot close")

    broken = BrokenClient(base_url="http://broken")
    healthy = FakeVLLMOnlineClient(base_url="http://healthy")
...
def test_retries_invalid_samples(tmp_path: Path) -> None:
    ...
    annotator = VLLMQueueAnnotator(
        clients=[FakeVLLMOnlineClient()], batch_size=4
    )
    result = annotator.run_annotation(
        output_dir=tmp_path / "out",
        prepared_dataset=_make_dataset(4),
